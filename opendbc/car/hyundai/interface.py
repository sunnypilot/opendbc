...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params_sp(stock_cp: structs.CarParams, ret: structs.CarParamsSP, candidate, fingerprint: dict[int, dict[int, int]],
    ...
    if not stock_cp.flags & HyundaiFlags.CANFD:
      ...
      if ESCC_MSG in fingerprint[0] or stock_cp.flags & HyundaiFlags.CANFD_ANGLE_STEERING:
  ...
  def init(CP, CP_SP, can_recv, can_send):
    if CP.openpilotLongitudinalControl and not ((CP.flags & (HyundaiFlags.CANFD_CAMERA_SCC | HyundaiFlags.CAMERA_SCC | HyundaiFlags.CANFD_ANGLE_STEERING)) or
