  ...
  def create_canfd_msgs(self, apply_steer_req, apply_torque, set_speed_in_units, accel, stopping, hud_control, CS, CC):
    ...
    if self.frame % 5 == 0 and (not lka_steering or lka_steering_long):
      if self.CP.flags & HyundaiFlags.CCNC:
        can_sends.extend(hyundaicanfd.create_ccnc(self.packer, self.CAN, self.CP.openpilotLongitudinalControl, CC.enabled, CC.hudControl, CC.latActive,
                                                  CC.leftBlinker, CC.rightBlinker, CS.msg_161, CS.msg_162, CS.is_metric, CS.main_cruise_enabled,
                                                  CS.out, self.lfa_icon))
      else:
    ...
    if self.CP.openpilotLongitudinalControl:
      ...
      if self.frame % 2 == 0:
        can_sends.append(hyundaicanfd.create_acc_control(self.packer, self.CAN, CC.enabled, self.accel_last, accel, stopping, CC.cruiseControl.override,
                                                         ...
                                                         CS.cruise_info if self.CP.flags & HyundaiFlags.CCNC else None))
