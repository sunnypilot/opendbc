...
class CarState(CarStateBase, CarStateExt):
  ...
  def update(self, can_parsers) -> tuple[structs.CarState, structs.CarStateSP]:
    ...
    if self.CP_SP.flags & ToyotaFlagsSP.SP_ENHANCED_BSM:
  ...
  def sp_get_enhanced_bsm(self, cp):
    left_status = int(cp.vl["BSM_RADAR"]["BSM_STATUS"])
    right_status = int(cp.vl["BSM_RADAR"]["BSM_RIGHT_STATUS"])
    return left_status not in (0, 254), right_status not in (0, 254)
