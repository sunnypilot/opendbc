...
class CarController(CarControllerBase, GasInterceptorCarController):
  def __init__(self, dbc_names, CP, CP_SP):
    ...
    self.reverse_cruise = bool(self.CP_SP.flags & ToyotaFlagsSP.REVERSE_CRUISE)
    ...
    if self.CP.openpilotLongitudinalControl:
      if self.frame % 3 == 0:
        ...
        can_sends.append(toyotacan.create_accel_command(self.packer, main_accel_cmd, pcm_cancel_cmd, self.permit_braking, self.standstill_req, lead,
                                                        CS.acc_type, fcw_alert, self.distance_button, reverse_cruise=self.reverse_cruise))
        if self.CP.flags & ToyotaFlags.SECOC.value:
      ...
      if pcm_cancel_cmd:
        if self.CP.carFingerprint in UNSUPPORTED_DSU_CAR:
          ...
          can_sends.append(toyotacan.create_accel_command(self.packer, 0, pcm_cancel_cmd, True, False, lead, CS.acc_type, False, self.distance_button,
                                                          reverse_cruise=self.reverse_cruise))
