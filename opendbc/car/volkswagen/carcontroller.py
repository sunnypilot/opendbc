    ...
    if self.frame % self.CCP.STEER_STEP == 0:
      if self.CP.flags & VolkswagenFlags.MEB:
        ...
        if CC.latActive:
          ...
          target_power = int(
            np.interp(CS.out.steeringTorque, [self.CCP.STEER_DRIVER_ALLOWANCE, self.CCP.STEER_DRIVER_MAX],
        ...
        if self.CP.flags & VolkswagenFlags.STOCK_HCA_PRESENT:
          ...
          can_sends.append(
            self.CCS.create_eps_update(self.packer_pt, CANBUS.cam, CS.eps_stock_values, ea_simulated_torque))
