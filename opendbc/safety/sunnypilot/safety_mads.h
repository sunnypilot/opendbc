...
inline void m_update_control_state(void) {
  ...
  if (m_mads_state.steering_disengage.transition == MADS_EDGE_RISING) {
...
inline void mads_state_update(const bool op_vehicle_moving, const bool op_acc_main, const bool op_allowed, const bool is_braking, const bool steering_disengage) {
  ...
  m_mads_state.steering_disengage.current = steering_disengage;
  ...
  m_update_binary_state(&m_mads_state.steering_disengage);
