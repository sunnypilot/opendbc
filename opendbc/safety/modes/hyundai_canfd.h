...
static bool hyundai_sp_ccnc = false;

static bool get_hyundai_sp_ccnc(void) {
  return hyundai_sp_ccnc;
}
...
static safety_config hyundai_canfd_init(uint16_t param) {
  ...
  const int HYUNDAI_PARAM_SP_CCNC = 16;
...
#define HYUNDAI_CANFD_LFA_STEERING_CAMERA_SCC_CCNC_TX_MSGS(longitudinal) \
  ...
  hyundai_sp_ccnc = GET_FLAG(param, HYUNDAI_PARAM_SP_CCNC);
