...
def is_steering_msg(mode, param, addr):
  ...
  if mode in (CarParams.SafetyModel.hondaNidec, CarParams.SafetyModel.hondaBosch):
    ...
    ret = addr == (0x110 if param & HyundaiSafetyFlags.CANFD_LKA_STEER_MSG_ALT else
                   ...
                   0xCB if param & HyundaiSafetyFlags.CANFD_ANGLE_STEERING else
                   0x12A)
...
def get_steer_value(mode, param, msg):
  ...
  if mode in (CarParams.SafetyModel.hondaNidec, CarParams.SafetyModel.hondaBosch):
    ...
    if param & HyundaiSafetyFlags.CANFD_ANGLE_STEERING and param & HyundaiSafetyFlags.CANFD_LKA_STEER_MSG:
      angle = (msg.data[11] << 6) | (msg.data[10] >> 2)
      angle = to_signed(angle, 14)
    elif param & HyundaiSafetyFlags.CANFD_ANGLE_STEERING:
