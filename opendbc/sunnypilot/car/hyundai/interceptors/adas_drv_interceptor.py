...
class AdasDrvEcuInterceptor(EcuInterceptorBase):
  ...
  def create_adas_drv_intercept_msg(self, packer, CAN, long_enabled):
    safety_param = HyundaiSafetyFlagsSP.ADAS_DRV_ECU_LAT_INTERCEPTOR  # I am assuming we always will do lat on interceptor

    if long_enabled:
      safety_param |= HyundaiSafetyFlagsSP.ADAS_DRV_ECU_LONG_INTERCEPTOR

    values = {
      "REQUESTED_SAFETY_MODE": SAFETY_MODE_ADAS_SAFETY_HKG_ADAS_DRV_INTERCEPTOR,
      "REQUESTED_SAFETY_PARAM": safety_param,
    }
