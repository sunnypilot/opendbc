...
def make_radar_track(track_id, d_rel, y_rel, motion_state=2, v_rel=0.0, age=2):
  return structs.CarControlSP.RadarTrack(
    trackId=track_id, dRel=d_rel, yRel=y_rel, vRel=v_rel, motionState=motion_state, age=age,
  )
...
class TestLeadDataCarController(unittest.TestCase):
  ...
  def test_cluster_radar_track_regions_and_motion_filter(self):
    ...
    tracks = [
      ...
      make_radar_track(7, 5, 0, motion_state=0),
      make_radar_track(8, 6, 0, motion_state=1),
    ]

    slots = selector.update(tracks)
  ...
  def test_cluster_radar_track_selection_is_stable(self):
    selector = ClusterRadarTrackSelector()
    self.assertEqual(selector.update([make_radar_track(1, 10, 0)])["lead"].trackId, 1)
    self.assertEqual(selector.update([make_radar_track(1, 12, 0), make_radar_track(2, 10, 0)])["lead"].trackId, 1)
    self.assertEqual(selector.update([make_radar_track(1, 12, 0), make_radar_track(2, 5, 0)])["lead"].trackId, 2)

  def test_cluster_radar_track_age_gate_keeps_neighbors_and_matched_leads_immediate(self):
    selector = ClusterRadarTrackSelector()

    self.assertEqual(selector.update([make_radar_track(1, 10, 0, age=1)]), {})
    self.assertEqual(selector.update([make_radar_track(2, 10, 3, age=1)])["lead_left"].trackId, 2)
    self.assertEqual(selector.update(
      [make_radar_track(3, 10, 0, age=1)], preferred_ids=(3,),
    )["lead"].trackId, 3)

  def test_cluster_radar_track_ignores_matched_stationary_lead(self):
    selector = ClusterRadarTrackSelector()
    tracks = [
      make_radar_track(1, 10, 0),
      make_radar_track(2, 20, 0),
      make_radar_track(3, 30, 0, motion_state=1),
    ]

    slots = selector.update(tracks, preferred_ids=(3,))

    self.assertEqual(slots["lead"].trackId, 1)
    self.assertNotIn("lead_alt", slots)

  def test_update_ccnc_cluster_tracks(self):
    msg_161 = {"DISTANCE_LEAD": 2}
    msg_162 = {}
    slots = {
    ...
    update_ccnc_cluster_tracks(msg_161, msg_162, slots, matched_track_ids={1})

    self.assertEqual(msg_161["DISTANCE_LEAD"], 0)
    self.assertEqual(msg_162["LEAD"], 2)
