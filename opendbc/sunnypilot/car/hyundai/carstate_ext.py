...
class CarStateExt:
  ...
  def update(self, ret: structs.CarState, can_parsers: dict[StrEnum, CANParser], speed_conv: float) -> None:
    ...
    if self.CP_SP.flags & HyundaiFlagsSP.NON_SCC:
      cruise_available_msg = "E_CRUISE_CONTROL" if self.CP.flags & HyundaiFlags.HYBRID else \
                             "EMS16"
      cruise_available_sig = "CRUISE_LAMP_M"

      cruise_enabled_msg = "E_CRUISE_CONTROL" if self.CP.flags & HyundaiFlags.HYBRID else \
                           "LVR12"
      cruise_enabled_sig = "CF_Lvr_CruiseSet"

      cruise_speed_msg = "ELECT_GEAR" if self.CP.flags & HyundaiFlags.HYBRID else \
                         "LVR12"
      cruise_speed_sig = "SLC_SET_SPEED" if self.CP.flags & HyundaiFlags.HYBRID else \
                         "CF_Lvr_CruiseSet"
