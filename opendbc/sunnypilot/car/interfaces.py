...
def _initialize_custom_longitudinal_tuning(CI: CarInterfaceBase, CP: structs.CarParams, CP_SP: structs.CarParamsSP, params) -> None:
...
def _initialize_neural_network_lateral_control(CI: CarInterfaceBase, CP: structs.CarParams, CP_SP: structs.CarParamsSP,
                                               params, enabled: bool = False) -> None:

  nnlc_model_path, nnlc_model_name, exact_match = get_nn_model_path(CP)

  if nnlc_model_name == "MOCK":
    carlog.error({"nnlc event": "car doesn't match any Neural Network model"})
...
def _initialize_radar_tracks(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params) -> None:
  if CP.brand == 'hyundai':
...
def setup_interfaces(CI: CarInterfaceBase, CP: structs.CarParams, CP_SP: structs.CarParamsSP, params) -> None:
...
def _enable_radar_tracks(CP: structs.CarParams, CP_SP: structs.CarParamsSP, can_recv: CanRecvCallable, params) -> None:
  if CP.brand == 'hyundai':
...
def init_interfaces(CP: structs.CarParams, CP_SP: structs.CarParamsSP, params, can_recv: CanRecvCallable, can_send: CanSendCallable):
