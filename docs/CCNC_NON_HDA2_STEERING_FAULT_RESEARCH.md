...
Status: passed on 2026-08-05 with the cleaned full-control design. The runtime no longer has lane/force/pass-through modes, companion-message fabrication, pre-arm delays, or a stock/openpilot handoff. While the camera-sync safety mode is selected, Panda always owns the rack-facing physical `0x12A`: a fresh safety-approved openpilot command supplies torque/request, and every disengaged, stale, malformed, or disallowed state supplies torque zero/request off. Stock steering is never the fallback.

The camera and car receive two internally consistent views:

- Car side: the physical camera's exact 100 Hz cadence/counter and unrelated payload, with only openpilot-owned torque/request/damping and the checksum changed. The real MDPS `0xEA` remains visible to the car.
- Camera side: the physical `0xEA` is preserved except `MDPS_LkaToiActvSta`, which is set to match the camera's original physical `0x12A ActToiSta`. Thus the camera receives the acknowledgement it expects even when its stock command is suppressed or openpilot steers without camera lanes.
- `0x1E0`, `0x1B5`, and `0x162` are not modified. The physical `0x161` is retained, with only `CENTERLINE` and `LFA_ICON` changed to show openpilot ownership. Stock warnings remain visible.

The first moving checkpoint covered 301.293 seconds after the initial engagement, 14 openpilot lateral transitions, 29 physical camera active-state transitions, and normalized openpilot torque reaching both full-scale directions. Every sampled `MDPS_LkaToiFltSta`, `MDPS_LkaFailSta`, `carState.steerFaultTemporary`, `FAULT_LSS`, `FAULT_LFA`, and `FAULT_DAS` value remained zero. Approximately 34 sampled seconds specifically covered the previously untested quadrant where the stock camera requested steering while openpilot was disengaged: physical camera torque continued changing, but real `MDPS_LkaToiActvSta` remained zero and the driver confirmed steering assistance disappeared immediately. Openpilot also steered with `ActToiSta=0`, proving lane-independent ownership.

The two-frame high-angle request reset remains enabled. During high-torque turns, the captured real MDPS acknowledgement briefly dropped and recovered without any fault, matching the separately validated guard behavior.

### Stock hands-on-wheel warning sequence

The stock camera continues its own driver-torque timer even though openpilot exclusively owns steering. Raw camera captures established the sequence: `ALERTS_2` changes from 0 to 1 (`KEEP_HANDS_ON_STEERING_WHEEL`), then about 15.12 seconds later to 2 (`KEEP_HANDS_ON_STEERING_WHEEL_RED`) while `SOUNDS_2` changes from 0 to 3 (`CONSTANT_CHIME`). If the driver still does not apply torque, the camera advances to `ALERTS_2=4`, `ALERTS_5=1`, and `SOUNDS_2=6`, then withdraws its physical `0x12A ActToiSta`. MDPS remains active under openpilot and all MDPS/`0x162` fault fields remain zero.

In the clean warning/SCC correlation, the initial warning began at monotonic 8585.086, the red warning began 15.148 seconds later, and the final shutdown began another 25.290 seconds later. The DBC labels `ALERTS_5=1` as `DRIVERS_GRASP_NOT_DETECTED_DRIVING_SPEED_WILL_BE_LIMITED`; it is not decoded as a steering-limit message. This car has `openpilotLongitudinalControl=false`. Stock camera `0x1A0` remained `MainMode_ACC=1`, `ACCMode=1` through the shutdown and continued issuing acceleration requests, including about -0.58 to -0.93 m/s2 afterward. A later transition to `ACCMode=4` occurred roughly 60 seconds after the shutdown and 34 seconds after the warning had cleared, so it cannot yet be attributed to the warning rather than driver input. The capture proves that SCC does not immediately cancel, but it does not yet rule out a separate downstream speed limit.

Low-speed testing is a poor environment for completing this timer: stock-like low damping leaves the wheel relatively loose, and bump-induced wheel oscillation can register as driver torque and reset the camera's grasp timer. Stop-and-go traffic also repeatedly changes lateral and SCC state. The decisive follow-up should use a clear road, retain all warnings, avoid driver torque through the final stage, and correlate actual vehicle acceleration/speed with `0x1A0`, MDPS state, openpilot lateral state, and `0x162` faults.

The driver's observation that the severe warning may be highway/HDA-specific is partly supported. Raw `0x161` disproves a highway-only explanation for the initial warnings: multiple `ALERTS_2=1` and `ALERTS_2=2` transitions occurred with both `HDA_ICON=0` and `NAV_ICON=0`. However, all three complete third-stage sequences in this capture began from `HDA_ICON=2`, `NAV_ICON=2`; `HDA_ICON` then cleared at or immediately after the shutdown while the camera withdrew LFA. The complete speed-limit stage may therefore be HDA-gated, but the current data cannot distinguish whether HDA state is a prerequisite or merely the only context that remained uninterrupted long enough. A clear-road, non-HDA 45-plus-second run is required.

An initial filter for the first two warning stages was deployed briefly, but was removed after the shutdown stage appeared and the user observed that stock SCC might also stop. Warnings are intentionally left intact while `0x1A0` SCC state is correlated. This vehicle has `openpilotLongitudinalControl=false`; any longitudinal shutdown in this test is stock SCC behavior, not an openpilot acceleration command. The `0x161` transform changes only `CENTERLINE` and `LFA_ICON`; it does not fabricate lane geometry or clear any alert, sound, or fault.

The branch also removed speculative combined `CANFD_LFA_CAMERA_SYNC + CANFD_ALT_BUTTONS` safety configurations because the tested Sonata uses the standard button message. Generic alt-button support for existing Hyundai platforms remains unchanged. The dynamic `BLINKERS.USE_ALT_LAMP` selection is retained because the test car transmitted `USE_ALT_LAMP=1`, and its physical lane changes failed until the alternate lamp bits were read.
...
2. Are other recent CCNC non-HDA2 Hyundai/Kia/Genesis platforms subject to the same strict stream behavior and camera-facing MDPS consistency check?
3. What are the semantics of the still-unnamed live bits in `0x11A`, `0x160`, `0x200`, `0x1FA`, and the undefined `0x38C` message?
4. What exact fixed-point filter phase/offset explains the last one-count damping residual when measured entirely inside Panda rather than across host-observed buses?
...
| `/Users/samaritan/Documents/ccnc-backups/2026-08-05-hands-warning/ccnc_hands_alert_20260805.jsonl` | Final 2,091-line camera/MDPS/SCC/openpilot correlation for the three-stage hands-on-wheel sequence, HDA/NAV context, and traffic resets | `f046ec2c9f4a4036d7c3ab20283e69ac3bfd2453d2e3171c746c9bfe0e3e49f0` |
...
The 2026-08-05 clean full-control run removed the disproven geometry, whole-frame, pre-arm, pass-through, and handoff variants. The minimal working behavior is now: retain Panda-timed physical `0x12A`, always replace its rack-facing steering fields with openpilot-or-zero, and make only the camera-facing two-bit MDPS active acknowledgement match the camera's original request. This provided steering with and without camera lanes, suppressed stock steering while disengaged, survived repeated camera/openpilot transitions and full torque, and kept every sampled MDPS and `0x162` fault clear. `0x162` remains completely untouched.
